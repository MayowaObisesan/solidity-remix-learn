...
contract PlayerClub {
    ...
    // function createLeague(string memory _name, string memory _country, string memory _clubName, uint _clubPosition) public {
    function createLeague(string memory _name, string memory _country) public {
        ...
        // league.position[_clubPosition] = Club(_clubName);
    }

    // function createPlayer(string memory _name, uint _clubPosition, string memory _clubName) public {
    function createPlayer(string memory _name, LeagueQuality _leagueQuality) public {
        player.name = _name;
        player.league.name = league.name;
        // player.league.position[_clubPosition] = Club(_clubName);
...
contract PlayerClub2 {
    ...
    function createPlayer(string memory _name, uint _clubPosition, string memory _clubName, LeagueQuality _leagueQuality) public {
        player.name = _name;
        player.league.name = league.name;
        player.league.position[_clubPosition] = Club(currentClub.standing++, _clubName);
        player.league.country = league.country;
        player.leagueQuality = _leagueQuality;
    }
